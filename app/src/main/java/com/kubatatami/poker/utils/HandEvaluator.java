...
public class HandEvaluator {
    ...
    private int[] suitDist = new int[Card.NO_OF_COLORS];
    ...
    private void calculateDistributions() {
        for (Card card : cards) {
            ...
            suitDist[card.getColor().ordinal()]++;
        }
    }
    ...
    private void findFlush() {
        for (int i = 0; i < Card.NO_OF_COLORS; i++) {
            if (suitDist[i] >= 5) {
                flushSuit = i;
                for (Card card : cards) {
                    if (card.getColor().ordinal() == flushSuit) {
    ...
    private boolean  isStraightFlush() {
        if (straightRank != -1 && flushRank == straightRank) {
            ...
            for (Card card : cards) {
                int rank = card.getRank();
                int suit = card.getColor().ordinal();
                if (lastRank != -1) {
